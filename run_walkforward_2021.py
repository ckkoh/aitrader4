"""
Walk-Forward Training Plan: Jan-Apr 2021
S&P 500 Trading System - Bull Market Test
"""
...
# Configuration
OUTPUT_DIR = Path('walkforward_2021_jan_apr')
...
MODEL_TYPE = 'xgboost'
CONFIDENCE_THRESHOLD = 0.55  # Standard threshold for bull market
HYPERPARAMETER_TUNING = True  # Enable for better performance (slower training)

# Date ranges for Rolling Window Strategy - 2021 Bull Market
ITERATIONS = [
    {
        'name': 'Jan_2021',
...
def get_2021_backtest_config():
    """Get backtest config optimized for 2021 bull market"""
    return BacktestConfig(
        initial_capital=10000.0,
        commission_pct=0.001,
        slippage_pct=0.0002,  # Lower for bull market (less volatility)
        position_size_pct=0.02,  # Standard for bull market
        max_position_value_pct=0.02,
        max_positions=1,
        max_daily_loss_pct=0.03,  # Slightly higher for trending market
        max_drawdown_pct=0.15,  # Standard
...
def main():
    print("="*70)
    print("WALK-FORWARD TRAINING PLAN: JAN-APR 2021")
    print("S&P 500 Trading System - Bull Market Test")
    print("="*70)
    print(f"\nTest Period: January-April 2021")
    print(f"Market Context: Bull market continuation, +12.26% buy & hold")
    print(f"Volatility: VIX 15-25 (moderate)")
    print(f"\nConfiguration:")
    ...
    # Get backtest config
    backtest_config = get_2021_backtest_config()
    ...
    print(f"Buy & Hold (Jan-Apr 2021): +12.26%")
    print(f"Excess Return: {total_return - 12.26:+.2f}%")
    ...
    if total_return > 12.26:
        print(f"✅ Beat buy & hold: PASS ({total_return:+.2f}% vs +12.26%)")
    else:
        print(f"❌ Beat buy & hold: FAIL ({total_return:+.2f}% vs +12.26%)")
    ...
    aggregate = {
        ...
        'buy_hold_return': 12.26,
        'excess_return': float(total_return - 12.26),
